...
export function HRMarkdown({ content, className = '' }: HRMarkdownProps) {
  return (
    <Markdown
      ...
      components={{
        p: ({ node, ...props }) => (
          <p className="text-base mb-3 last:mb-0" {...props} />
        ),
        ul: ({ node, ...props }) => (
        ...
        h5: ({ node, ...props }) => (
          <h5 className="text-base font-medium mt-2 mb-1 first:mt-0" {...props} />
        ),
        h6: ({ node, ...props }) => (
        ...
        blockquote: ({ node, ...props }) => (
          <blockquote
            className="border-l-4 border-gray-300 pl-4 py-2 italic my-4"
            {...props}
          />
        ),
        a: ({ node, ...props }) => (
